...
type MsgHandle interface {
	Handle(c *Connector, msg *PduMsg) error
}

//type UnknownMsgHandle interface {
//	UnknownMsgHandle(c *Connector, msg *PduMsg) error
//}

//type UnknownRespMsgHandle interface {
//	UnknownRespMsgHandle(c *Connector, msg *PduMsg) error
//}
...
type MsgFunc func(c *Connector, msg *PduMsg) error

func (f MsgFunc) Handle(c *Connector, msg *PduMsg) error {
	return f(c, msg)
}
...
type Connector struct {
	...
	userdata   interface{} // save user data
	sessions   SessionMap  //local request session wait for remote response
	handler    MsgHandle   //remote request to local handle
	msgpool    MsgPool     //msg pool
}
...
type Session interface {
	...
	Wait(timeout time.Duration) (msg *PduMsg, err error) //wait for reply
	_reply(msg *PduMsg, err error)                       //call by underline for reply event
}
type SessionMap interface {
	NewSession(msg *PduMsg) Session
	EndSession(msg *PduMsg) error
}
...
func NewConnector(conn net.Conn) *Connector {
	...
	rt.sendevent = make(chan *PduMsg, 1000)
...
func (cn *Connector) GetUserData() interface{} {
	return cn.userdata
}

func (cn *Connector) SetUserData(data interface{}) {
...
func (cn *Connector) rcvRoutine() {
	...
	for {
		...
		//var header PduHead
		newmsg := cn.msgpool.GetPdu()
		_, err := newmsg.Header.ReadFromIO(reader)
		if err != nil {
			cn.emitEror(fmt.Errorf("read header err %s", err))
			cn.msgpool.PutPdu(newmsg)
			return
		}

		_, err = newmsg.Body.ReadFromIO(reader, int(newmsg.Header.GetPduLen()))
		if err != nil {
		...
		cn.Log("rcv", newmsg)
		cmd := newmsg.Header.GetCommandID()
		if cmd.IsReqCommand() == false {
			cn.sessions.EndSession(newmsg)
			//			issession := func() bool {
			//				//cn.sessions.lock.Lock()
			//				//defer cn.sessions.lock.Unlock()
			//				rspchan, ok := cn.sessions.smap[newmsg.Header.Seq()]
			//				if ok {
			...
			//					delete(cn.sessions.smap, newmsg.Header.Seq())
			//					return true
			//				}
			//				return false
			//			}()
...
func (cn *Connector) sendRequst(msg *PduMsg, timeout time.Duration) (rsp *PduMsg, err error) {
	...
	s := cn.sessions.NewSession(msg)
	cn.sendevent <- msg
	rt, err := s.Wait(timeout)
	s.Close()
	return rt, err
}

...
type Client struct {
	...
	//out     MChan
	//seq     uint32
	//waitRsp map[uint32]chan CommMsg
...
//func (ch *MChan) GetChan() chan CommMsg {
//	ch.lock.Lock()
...
func (cli *Client) Handle(cn *Connector, msg *CommMsg) error {
...
func (cli *Client) Open() error {
	...
	body := NewBind(&cli.option.Comm_bind)
	header.SetPduLen(uint32(len(body.Data())))
	header.SetCommandID(cli.option.Bind_Type)
	//header.SetSeq(cli.NewSeq())

	rsp, err := cli.conn.SendRequst(CommMsg{Header: header, Body: *body}, time.Second*2)
...
func (cl *Client) UnknownMsgHandle(cn *Connector, msg *CommMsg) error {

	cn.ReplyfromMsg(msg.Header, KNOWN_MSG_TYPE, Body{})
	return nil
}

func (cl *Client) UnknownRespMsgHandle(cn *Connector, msg *CommMsg) error {
	return nil
}

//func (cli *Client) send(header SmppHeader, body SmppBody) {
//	msg := CommMsg{header, body}
//	//sync.Pool.Get().(*CommMsg)

...
func (svr *Server) RegHandle(cmd uint32, handle MsgHandle) {
	...
	case CLIENT_BIND_REC, CLIENT_BIND:

		wrap := func(cn *Connector, msg *CommMsg) error {
...
func (svr *Server) Handle(cn *Connector, msg *CommMsg) error {
...
func (svr *Server) UnknownMsgHandle(cn *Connector, msg *CommMsg) error {
	cn.ReplyfromMsg(msg.Header, KNOWN_MSG_TYPE, Body{})
	return ERROR_NOFINDHANDLE
}

func (svr *Server) UnknownRespMsgHandle(cn *Connector, msg *CommMsg) error {
	return nil
}
...
//			cn.rcvevent <- CommMsg{header, body}
//		}

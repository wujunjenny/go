...
type MsgHandle interface {
	Handle(c *Connector, msg *CommMsg) error
}

type UnknownMsgHandle interface {
	UnknownMsgHandle(c *Connector, msg *CommMsg) error
}

type UnknownRespMsgHandle interface {
	UnknownRespMsgHandle(c *Connector, msg *CommMsg) error
}

type MsgFunc func(c *Connector, msg *CommMsg) error

func (f MsgFunc) Handle(c *Connector, msg *CommMsg) error {
	return f(c, msg)
}
...
type Connector struct {
	conn net.Conn
	seq  uint32
	//rcvevent   chan CommMsg
	sendevent  chan CommMsg
	done       chan struct{}
	sessions   sessionmap
	lasterror  error
	status     int32
...
func NewConnector(conn net.Conn) *Connector {
	...
	//rt.rcvevent = make(chan CommMsg, 1000)
	rt.sendevent = make(chan CommMsg, 1000)
...
func (cn *Connector) rcvRoutine() {
	...
	for {
		...
		if IsReqCommand(cmd) == false {
			issession := func() bool {
				...
				if ok {
					...
					go func() {
						rspchan.ch <- CommMsg{header, body}
						close(rspchan.ch)
					}()
			...
			if issession {
				cn.Log("resp ok continue")
				continue
			} else {
				cn.Log("resp err continue")
				if h, ok := cn.owner.(UnknownRespMsgHandle); ok {
					h.UnknownRespMsgHandle(cn, &CommMsg{header, body})
				}
		...
		if f, ok := cn.owner.(MsgHandle); ok {
			cn.Log("req")
			f.Handle(cn, &CommMsg{header, body})
			continue
...
func (cn *Connector) SendRequst(msg CommMsg, timeout time.Duration) (rsp *CommMsg, err error) {
...
func (cn *Connector) BindReq(bdtype uint32, bd *Comm_bind) (rsp *CommMsg, err error) {
	var msg CommMsg
...
func (cn *Connector) RequstSm(smtype uint32, sm *Smpp_submit_sm, args ...TLV) (rsp *CommMsg, err error) {
	var msg CommMsg
...
func (cn *Connector) ReplyfromMsg(srchd Header, rtcode uint32, rep Body) (err error) {
	...
	rply.Body = rep
	rply.Header.SetCommandID(srchd.GetCommandID() | 0x8000)
